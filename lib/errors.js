...
/**
 * An error that is used when Gogs answers the request with a 304 code,
 * indicating that the API URL has been permanently moved to somewhere
 * else. This should, ideally, never happen as I should keep up-to-date
 * with the API.
 *
 * @extends {GogsCliError}
 */
class MovedPermanently extends GogsCliError {
  constructor(url, location) {
    super(`The request was trying to reach ${url}, ` +
          `but was redirected to ${location} permanently. ` +
          'This should not have happened and can mean that ' +
          'the API URLs have changed.');
  }
}

class Found extends GogsCliError {
  constructor(url, location) {
    super(`The request was trying to reach ${url}, ` +
          `but was redirected to ${location}. ` +
          'This should not have happened and can mean that ' +
          'the API URLs have changed.');
  }
}
...
/**
 * An error that indicates that it cannot connect to the Gogs host
 * specified by hostname. This is often due to either the user entering
 * the incorrect url or the host being inaccessible.
 *
 * @extends {GogsCliError}
 */
class CannotConnect extends GogsCliError {
  constructor(hostname) {
    super(`The Gogs CLI could not connect to the host at "${hostname}"`);
  }
}
