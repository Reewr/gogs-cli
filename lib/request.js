...
const getDefaultError = (statusCode, url) => {
  switch (statusCode) {
    case 400: return new error.InvalidAccess();
    case 401: return new error.InvalidRequest();
    case 404: return new error.NotFound('Resource', url);
...
const performRequest = function(method, url, data = null) {
  ...
  const req = request(options, res => {
    ...
    if (res.statusCode > 299 || res.statusCode < 200) {
      ...
      if (!hadHandler)
        emitter.emit('error', getDefaultError());
    }
